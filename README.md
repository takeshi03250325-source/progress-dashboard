...
`config/news-sources.yaml` に RSS を列挙し、`npm run morning:fetch` で取得・スコアリング・1 件選定と投稿案を `output/morning-run.json` に書き出します。続けて `npm run morning:line` で Messaging API のプッシュ（要 `.env` の `LINE_CHANNEL_ACCESS_TOKEN` と `LINE_PUSH_TO_USER_ID`）。GitHub Actions では [`.github/workflows/morning-news.yml`](.github/workflows/morning-news.yml) が平日朝（JST）に同じ処理を実行します。リポジトリの **Secrets** に同名の変数を登録してください。
